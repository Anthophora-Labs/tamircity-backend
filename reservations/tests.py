...
class PostUpdateDelete(APITestCase):
    ...
    def test_post_update(self):
        ...
        self.assertEqual(200, response.status_code)
        self.assertTrue(Reservation.objects.get(id=self.post.id).content == data["content"])
    ...
    def test_post_update_different_user(self):
        ...
        response = self.client.put(self.url, data)
        self.assertEqual(403, response.status_code)
        self.assertFalse(Reservation.objects.get(id=self.post.id).content == data["content"])
